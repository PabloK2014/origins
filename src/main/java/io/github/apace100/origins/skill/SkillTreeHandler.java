...
public class SkillTreeHandler {
    ...
    public static void initializeSkillTrees() {
        ...
        CLASS_SKILL_TREES.put("origins:warrior", warTree);

        // Класс Курьер
        SkillTree courierTree = new SkillTree();

        // Ветка "Скорость"
        List<Skill> speedBranch = Arrays.asList(
                new Skill("speed_basic", "Базовая скорость", "Увеличивает скорость передвижения на 5% за уровень", SkillType.PASSIVE, 3, 1, null),
                new Skill("hunger_reduction", "Снижение голода", "Снижает расход голода на 10% за уровень", SkillType.PASSIVE, 3, 2, "speed_basic"),
                new Skill("sprint_boost", "Рывок", "Активирует временное ускорение на 10 секунд", SkillType.ACTIVE, 1, 3, "hunger_reduction"),
                new Skill("speed_surge", "Всплеск скорости", "Мощное ускорение на 30 секунд с восстановлением голода", SkillType.GLOBAL, 1, 5, "sprint_boost")
        );
        ...
        courierTree.addBranch(inventoryBranch);
        courierTree.addBranch(carryBranch);
        CLASS_SKILL_TREES.put("origins:courier", courierTree);
        ...
        List<Skill> miningBranch = Arrays.asList(
                new Skill("ore_double", "Удвоение руды", "Шанс получить двойную руду", SkillType.PASSIVE, 5, 5, null),
        ...
        List<Skill> explorationBranch = Arrays.asList(
                new Skill("ore_drop_chance", "Случайная находка", "Шанс найти редкие предметы", SkillType.PASSIVE, 5, 3, null),
        ...
        List<Skill> survivalBranch = Arrays.asList(
                ...
                new Skill("deep_miner_global", "Глубинный шахтёр", "Увеличивает добычу ресурсов в глубоких шахтах на 25%", SkillType.GLOBAL, 1, 5, "torch_range")
        );

        minerTree.addBranch(miningBranch);
        minerTree.addBranch(explorationBranch);
        minerTree.addBranch(survivalBranch);
        CLASS_SKILL_TREES.put("origins:miner", minerTree);

        // Класс Пивовар
        SkillTree brewerTree = new SkillTree();
        ...
        brewerTree.addBranch(fermentationBranch);
        brewerTree.addBranch(drunkMasterBranch);
        brewerTree.addBranch(tavernPartyBranch);
        CLASS_SKILL_TREES.put("origins:brewer", brewerTree);
        ...
        CLASS_SKILL_TREES.put("origins:cook", cookTree);
        ...
        CLASS_SKILL_TREES.put("origins:blacksmith", smithTree);
    }
